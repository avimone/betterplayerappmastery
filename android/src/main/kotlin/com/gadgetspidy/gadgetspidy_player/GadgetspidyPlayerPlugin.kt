...
package com.gadgetspidy.gadgetspidy_player
...
import com.gadgetspidy.gadgetspidy_player.GadgetspidyPlayerCache.releaseCache
 ...
 */
class GadgetspidyPlayerPlugin : FlutterPlugin, ActivityAware, MethodCallHandler {
    private val videoPlayers = LongSparseArray<GadgetspidyPlayer>()
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        if (flutterState == null || flutterState?.textureRegistry == null) {
            result.error("no_activity", "gadgetspidy_player plugin requires a foreground activity", null)
            return
        }
        when (call.method) {
    ...
    private fun stopPreCache(call: MethodCall, result: MethodChannel.Result) {
        ...
        GadgetspidyPlayer.stopPreCache(flutterState?.applicationContext, url, result)
    }

    private fun clearCache(result: MethodChannel.Result) {
        GadgetspidyPlayer.clearCache(flutterState?.applicationContext, result)
    }

    private fun getTextureId(gadgetspidyPlayer: GadgetspidyPlayer): Long? {
        for (index in 0 until videoPlayers.size()) {
            if (gadgetspidyPlayer === videoPlayers.valueAt(index)) {
    ...
    private fun setupNotification(gadgetspidyPlayer: GadgetspidyPlayer) {
        try {
            val textureId = getTextureId(gadgetspidyPlayer)
            if (textureId != null) {
                ...
                if (showNotification) {
                    ...
                    gadgetspidyPlayer.setupPlayerNotification(
                        flutterState?.applicationContext!!,
...
private fun enablePictureInPicture(player: GadgetspidyPlayer) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        try {
            ...
            // Support seamless resize for Gadgetspidy transition from fullscreen
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
...
private fun createPipActions(): List<RemoteAction> {
    ...
    val playPauseIntent = Intent("GADGETSPIDY_PLAYER_PIP_CONTROL")
...
private fun disablePictureInPicture(player: GadgetspidyPlayer) {
    try {
        stopPipHandler()
...
private fun startPictureInPictureListenerTimer(player: GadgetspidyPlayer) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
...
private fun updatePipActions() {
    ...
    private fun dispose(player: GadgetspidyPlayer, textureId: Long) {
        player.dispose()
...
private fun stopPipHandler() {
    ...
    private class FlutterState(
        ...
        fun startListening(methodCallHandler: GadgetspidyPlayerPlugin?) {
    ...
    companion object {
        private const val TAG = "GadgetspidyPlayerPlugin"
        private const val CHANNEL = "gadgetspidy_player_channel"
        private const val EVENTS_CHANNEL = "gadgetspidy_player_channel/videoEvents"
