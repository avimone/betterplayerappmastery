...
import 'package:gadgetspidy_player/src/controls/gadgetspidy_player_cupertino_progress_bar.dart';
...
import '../configuration/gadgetspidy_player_configuration.dart';

class GadgetspidyPlayerCupertinoControls extends StatefulWidget {
  ...
  const GadgetspidyPlayerCupertinoControls({
  ...
  State<StatefulWidget> createState() {
    return _GadgetspidyPlayerCupertinoControlsState();
  }
}

class _GadgetspidyPlayerCupertinoControlsState
    extends GadgetspidyPlayerControlsState<GadgetspidyPlayerCupertinoControls> {
  ...
  Widget _buildMainWidget() {
    ...
    final isFullScreen = _gadgetspidyPlayerController?.isFullScreen == true;
  ...
  Widget _buildPosition() {
    ...
    return Padding(
      ...
      child: Text(
        GadgetspidyPlayerUtils.formatDuration(position),
        style: TextStyle(
      ...
      child: Container(
        ...
        child: ClipRRect(
          ...
          child: Container(
            height: barHeight,
            decoration: BoxDecoration(color: backgroundColor),
            child: _gadgetspidyPlayerController!.isLiveStream()
                ? Row(
...
// Handle fullscreen button exit behavior for Cupertino
  ...
  Widget _buildProgressBar() {
    return Expanded(
      child: Padding(
        padding: const EdgeInsets.only(right: 12.0),
        child: GadgetspidyPlayerCupertinoVideoProgressBar(
          _controller,
          _gadgetspidyPlayerController,
          onDragStart: () {
            _hideTimer?.cancel();
  ...
  void _onPlayPause() {
    ...
    if (_controller!.value.isPlaying) {
      ...
      if (!_controller!.value.initialized) {
        if (_gadgetspidyPlayerController!
                .gadgetspidyPlayerDataSource?.liveStream ==
            true) {
  ...
  void _onCupertinoPipButtonPressed() async {
    if (_gadgetspidyPlayerController?.gadgetspidyPlayerGlobalKey != null) {
      try {
        ...
        if (_gadgetspidyPlayerController!.isPipActive) {
          await _gadgetspidyPlayerController!.disablePictureInPicture();
        } else {
          await _gadgetspidyPlayerController!.enablePictureInPicture(
              _gadgetspidyPlayerController!.gadgetspidyPlayerGlobalKey!);
        }
      } catch (e) {
        GadgetspidyPlayerUtils.log("Failed to toggle Picture in Picture: $e");
