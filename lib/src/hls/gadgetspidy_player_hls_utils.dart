...
import 'package:gadgetspidy_player/src/asms/gadgetspidy_player_asms_data_holder.dart';
import 'package:gadgetspidy_player/src/asms/gadgetspidy_player_asms_subtitle.dart';
import 'package:gadgetspidy_player/src/asms/gadgetspidy_player_asms_subtitle_segment.dart';
import 'package:gadgetspidy_player/src/asms/gadgetspidy_player_asms_track.dart';
import 'package:gadgetspidy_player/src/asms/gadgetspidy_player_asms_utils.dart';
import 'package:gadgetspidy_player/src/core/gadgetspidy_player_utils.dart';
import 'package:gadgetspidy_player/src/hls/hls_parser/hls_master_playlist.dart';
import 'package:gadgetspidy_player/src/hls/hls_parser/hls_media_playlist.dart';
import 'package:gadgetspidy_player/src/hls/hls_parser/hls_playlist_parser.dart';
import 'package:gadgetspidy_player/src/hls/hls_parser/rendition.dart';
import 'package:gadgetspidy_player/src/hls/hls_parser/segment.dart';
import 'package:gadgetspidy_player/src/hls/hls_parser/util.dart';

///HLS helper class
class GadgetspidyPlayerHlsUtils {
  static Future<GadgetspidyPlayerAsmsDataHolder> parse(
    ...
    try {
      ...
      tracks = list[0] as List<GadgetspidyPlayerAsmsTrack>;
      subtitles = list[1] as List<GadgetspidyPlayerAsmsSubtitle>;
      audios = list[2] as List<GadgetspidyPlayerAsmsAudioTrack>;
    } catch (exception) {
      GadgetspidyPlayerUtils.log("Exception on hls parse: $exception");
    }
    return GadgetspidyPlayerAsmsDataHolder(
        tracks: tracks, audios: audios, subtitles: subtitles);
  }

  static Future<List<GadgetspidyPlayerAsmsTrack>> parseTracks(
      String data, String masterPlaylistUrl) async {
    final List<GadgetspidyPlayerAsmsTrack> tracks = [];
    try {
      ...
      if (parsedPlaylist is HlsMasterPlaylist) {
        parsedPlaylist.variants.forEach(
          (variant) {
            tracks.add(GadgetspidyPlayerAsmsTrack('', variant.format.width,
      ...
      if (tracks.isNotEmpty) {
        tracks.insert(0, GadgetspidyPlayerAsmsTrack.defaultTrack());
      }
    } catch (exception) {
      GadgetspidyPlayerUtils.log("Exception on parseSubtitles: $exception");
    }
    return tracks;
  }

  ///Parse subtitles from provided m3u8 url
  static Future<List<GadgetspidyPlayerAsmsSubtitle>> parseSubtitles(
      String data, String masterPlaylistUrl) async {
    final List<GadgetspidyPlayerAsmsSubtitle> subtitles = [];
    try {
      final parsedPlaylist = await HlsPlaylistParser.create()
          .parseString(Uri.parse(masterPlaylistUrl), data);
  ...
  ///on time in playlist.
  static Future<GadgetspidyPlayerAsmsSubtitle?> _parseSubtitlesPlaylist(
      Rendition rendition) async {
    try {
      final HlsPlaylistParser _hlsPlaylistParser = HlsPlaylistParser.create();
      final subtitleData = await GadgetspidyPlayerAsmsUtils.getDataFromUrl(
      ...
      final List<GadgetspidyPlayerAsmsSubtitleSegment> asmsSegments = [];
      ...
      for (final Segment segment in hlsMediaPlaylist.segments) {
        ...
        if (isSegmented) {
          ...
          asmsSegments.add(
            GadgetspidyPlayerAsmsSubtitleSegment(
      ...
      return GadgetspidyPlayerAsmsSubtitle(
  ...
  static Future<List<GadgetspidyPlayerAsmsAudioTrack>> parseLanguages(
      String data, String masterPlaylistUrl) async {
    final List<GadgetspidyPlayerAsmsAudioTrack> audios = [];
    final parsedPlaylist = await HlsPlaylistParser.create()
        .parseString(Uri.parse(masterPlaylistUrl), data);
    if (parsedPlaylist is HlsMasterPlaylist) {
      for (int index = 0; index < parsedPlaylist.audios.length; index++) {
        final Rendition audio = parsedPlaylist.audios[index];
        audios.add(GadgetspidyPlayerAsmsAudioTrack(
          id: index,
          label: audio.name,
