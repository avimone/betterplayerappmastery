...
@implementation GadgetspidyPlayer
...
- (nonnull UIView *)view {
    GadgetspidyPlayerView *playerView = [[GadgetspidyPlayerView alloc] initWithFrame:CGRectZero];
...
- (void)setDataSourceURL:(NSURL*)url withKey:(NSString*)key withCertificateUrl:(NSString*)certificateUrl withLicenseUrl:(NSString*)licenseUrl withHeaders:(NSDictionary*)headers withCache:(BOOL)useCache cacheKey:(NSString*)cacheKey cacheManager:(CacheManager*)cacheManager overriddenDuration:(int) overriddenDuration videoExtension: (NSString*) videoExtension{
    ...
    if (useCache){
        ...
        if (certificateUrl && certificateUrl != [NSNull null] && [certificateUrl length] > 0) {
            ...
            _loaderDelegate = [[GadgetspidyPlayerEzDrmAssetsLoaderDelegate alloc] init:certificateNSURL withLicenseURL:licenseNSURL];
    ...
    if (context == timeRangeContext) {
        if (_eventSink != nil) {
            ...
            for (NSValue* rangeValue in [object loadedTimeRanges]) {
                ...
                int64_t start = [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:(range.start)];
                int64_t end = start + [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:(range.duration)];
                if (!CMTIME_IS_INVALID(_player.currentItem.forwardPlaybackEndTime)) {
                    int64_t endTime = [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:(_player.currentItem.forwardPlaybackEndTime)];
                    if (end > endTime){
                        end = endTime;
                    }
                }
...
- (void)onReadyToPlay {
    if (_eventSink && !_isInitialized && _key) {
        ...
        int64_t duration = [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:(_player.currentItem.asset.duration)];
...
- (int64_t)position {
    return [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:([_player currentTime])];
}

- (int64_t)absolutePosition {
    return [GadgetspidyPlayerTimeUtils FLTNSTimeIntervalToMillis:([[[_player currentItem] currentDate] timeIntervalSince1970])];
}
...
- (int64_t)duration {
    ...
    return [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:(time)];
}
    ...
    if (@available(iOS 9.0, *)) {
        @try {
            ...
            if (!_pipController && self._playerLayer && 
                ...
                // Enable requires linear playback (iOS 14.0+) for Gadgetspidy HLS support
...
- (void) enablePictureInPicture: (CGRect) frame {
    @try {
        ...
        // NEW: Get reference to the original GadgetspidyPlayerView
        GadgetspidyPlayerView* originalPlayerView = (GadgetspidyPlayerView*)self.view;
...
// Enhanced delegate methods with Gadgetspidy error handling
