...
@objc public class CacheManager: NSObject {
    ...
    var diskConfig = DiskConfig(name: "GadgetspidyPlayerCache", expiry: .date(Date().addingTimeInterval(3600*24*30)),
    ...
    @objc public func setMaxCacheSize(_ maxCacheSize: NSNumber?){
        if let unsigned = maxCacheSize {
            ...
            diskConfig = DiskConfig(name: "GadgetspidyPlayerCache", expiry: .date(Date().addingTimeInterval(3600*24*30)), maxSize: _maxCacheSize)
        }        
    }
