...
#import "GadgetspidyPlayerPlugin.h"
#import <gadgetspidy_player/gadgetspidy_player-Swift.h>
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    [FlutterMethodChannel methodChannelWithName:@"gadgetspidy_player_channel"
                                binaryMessenger:[registrar messenger]];
    GadgetspidyPlayerPlugin* instance = [[GadgetspidyPlayerPlugin alloc] initWithRegistrar:registrar];
    [registrar addMethodCallDelegate:instance channel:channel];
    //[registrar publish:instance];
    [registrar registerViewFactory:instance withId:@"com.jhomlala/gadgetspidy_player"];
}
...
- (void)detachFromEngineForRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    for (NSNumber* textureId in _players.allKeys) {
        GadgetspidyPlayer* player = _players[textureId];
...
- (NSObject<FlutterPlatformView>*)createWithFrame:(CGRect)frame
    ...
    GadgetspidyPlayerView* player = [_players objectForKey:@(textureId.intValue)];
    return player;
}
...
#pragma mark - GadgetspidyPlayerPlugin class
- (int)newTextureId {
    texturesCount += 1;
    return texturesCount;
}
- (void)onPlayerSetup:(GadgetspidyPlayer*)player
               result:(FlutterResult)result {
    int64_t textureId = [self newTextureId];
    FlutterEventChannel* eventChannel = [FlutterEventChannel
                                         eventChannelWithName:[NSString stringWithFormat:@"gadgetspidy_player_channel/videoEvents%lld",
                                                               textureId]
...
- (void) setupRemoteNotification :(GadgetspidyPlayer*) player{
...
- (void) setupRemoteCommands:(GadgetspidyPlayer*)player  {
    ...
    if (@available(iOS 9.1, *)) {
        [commandCenter.changePlaybackPositionCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent * _Nonnull event) {
            if (_notificationPlayer != [NSNull null]){
                ...
                int64_t millis = [GadgetspidyPlayerTimeUtils FLTCMTimeToMillis:(time)];
...
- (void) setupRemoteCommandNotification:(GadgetspidyPlayer*)player, NSString* title, NSString* author , NSString* imageUrl{
...
- (NSString*) getTextureId: (GadgetspidyPlayer*) player{
    NSArray* temp = [_players allKeysForObject: player];
    NSString* key = [temp lastObject];
    return key;
}

- (void) setupUpdateListener:(GadgetspidyPlayer*)player,NSString* title, NSString* author,NSString* imageUrl  {
...
- (void) disposeNotificationData: (GadgetspidyPlayer*)player{
...
- (void) stopOtherUpdateListener: (GadgetspidyPlayer*) player{
    ...
    for (NSString* textureId in _timeObserverIdDict.allKeys) {
        ...
        GadgetspidyPlayer* playerToRemoveListener = [_players objectForKey:textureId];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"init" isEqualToString:call.method]) {
        ...
        GadgetspidyPlayer* player = [[GadgetspidyPlayer alloc] initWithFrame:CGRectZero];
